...
//   // Shared batch for all WebSocket connections
//   Map<String, dynamic> _tempJackpotBatch = {};
//   // Flag to track if a batch has been marked as finished
//   bool _isBatchFinished = false;
//   // Required jackpot names
...
//     }
//     // Random delay between 0-2 seconds to stagger reconnections
//     await Future.delayed(Duration(milliseconds: 100 + (Random().nextInt(3500))));
//     try {
//       debugPrint('JackpotPriceBloc: Connecting to WebSocket ${ConfigCustom.webSocketEndpoints[index]}');
//       _channels[index] = IOWebSocketChannel.connect(ConfigCustom.webSocketEndpoints[index],pingInterval: const Duration(seconds: 30)); // Send client pings);
...
//             final String level = data['Id'].toString();
...
//               }
//               // Add to shared batch
//               _tempJackpotBatch[key] = value;
//               // debugPrint('JackpotPriceBloc: Added $key: $value to tempJackpotBatch');
//               // debugPrint('JackpotPriceBloc: Current tempJackpotBatch: $_tempJackpotBatch');
//               // Check if batch contains all 9 required jackpot names
//                 if (_tempJackpotBatch.keys.toSet().containsAll(ConfigCustom.selectedJackpotNames)) {
//                   // debugPrint('JackpotPriceBloc: FinishedTempJackpotBatch: $_tempJackpotBatch');
//                   try {
//                     await hiveService.appendJackpotHistory(_tempJackpotBatch.cast<String, double>());
//                     // debugPrint('JackpotPriceBloc: Saved batch to Hive: $_tempJackpotBatch');
//                   } catch (e) {
...
//     // debugPrint('JackpotPriceBloc: Update event for level ${event.level} ignored, processing in WebSocket listener');
//   }
